...
int CreateBMPFile(HWND hwnd, LPTSTR pszFile, PBITMAPINFO pbi,  HBITMAP hBMP, HDC hDC) {
    ...
    if (!GetDIBits(hDC, hBMP, 0, (WORD) pbih->biHeight, lpBits, pbi, DIB_RGB_COLORS)) { return 1; }
	...
	if (!WriteFile(hf, (LPVOID) &hdr, sizeof(BITMAPFILEHEADER), (LPDWORD) &dwTmp,  NULL)) { return 1; }
	
    // Copy the BITMAPINFOHEADER and RGBQUAD array into the file.  
	if (!WriteFile(hf, (LPVOID) pbih, sizeof(BITMAPINFOHEADER) + pbih->biClrUsed * sizeof (RGBQUAD), (LPDWORD) &dwTmp, ( NULL))) { return 1; }

    // Copy the array of color indices into the .BMP file.  
    /*dwTotal =*/ cb = pbih->biSizeImage; 
    hp = lpBits; 
	if (!WriteFile(hf, (LPSTR) hp, (int) cb, (LPDWORD) &dwTmp, NULL)) { return 1; }

    // Close the .BMP file.  
    if (!CloseHandle(hf)) { return 1; }
