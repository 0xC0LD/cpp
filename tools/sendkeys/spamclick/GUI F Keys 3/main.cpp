// TODO: 
// 1. add ctrl+a https://stackoverflow.com/questions/10127054/select-all-text-in-edit-contol-by-clicking-ctrla
// 2. prettier button (with borders, ...)
// 3. make faster sendkeys.cpp code (update: sendkey, spamkey, bloatedshell)
// 4. textbox redraw on keypress (fix the 'filled with chars' draw bug)
// 5. add DEBUG macro (remove the perror code)
// 6. remove iostream
...
const unsigned int controls_count = 6;
...
void updateUI(const unsigned int& uiNum) {
	switch (uiNum) {
		...
		case 5: SetWindowText(controls_status[5], std::to_string(spamDelay).c_str()); break;
		
		case 10: SetWindowText(controls_counters[0], std::to_string(LMB_clicks).c_str()); break;
		case 11: SetWindowText(controls_counters[1], std::to_string(RMB_clicks).c_str()); break;
		case 12: SetWindowText(controls_counters[2], std::to_string(cu1_clicks).c_str()); break;
...
static HBRUSH color_fg = CreateSolidBrush(RGB(255, 255, 255));
static HBRUSH color_bg = CreateSolidBrush(RGB(10, 10, 10));
